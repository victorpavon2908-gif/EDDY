...
class LocalBrain {
    ...
    private fun understandSingle(input: String): AssistantCommand {
        ...
        if (containsAny(text, "configura inteligencia", "configurar inteligencia", "configura ia", "configurar ia", "configura busqueda web", "configurar busqueda web", "configura internet de niko")) return AssistantCommand.OpenAiSettings
        ...
        if (Regex("^(?:niko|nico|hola|como estas|oye niko|buenos dias|buenas tardes|buenas noches)[.!?]*$").matches(text)) return AssistantCommand.Greeting
        return AssistantCommand.Unknown(original)
    }
    ...
    private fun parseDynamicTool(text: String): AssistantCommand? {
        if (
            containsAny(text, "vuelve a tu pantalla principal", "vuelve a la pantalla principal", "regresa a tu pantalla principal", "regresa a la pantalla principal", "vuelve a inicio", "regresa a inicio", "volver a niko", "regresar a niko")
        ) return AssistantCommand.OpenAppByName("NIKO_HOME")
