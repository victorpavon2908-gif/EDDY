...
/**
 * Registro local de capacidades de NIKO. Los skills declarativos pueden aprenderse y
 * activarse sin reemplazar el APK. El código nativo sigue pasando por build/firma.
 */
class NikoSkillEngine(context: Context) {
    private val appContext = context.applicationContext
    private val root = File(appContext.filesDir, UpgradeIdentity.skillsDirectory).apply { mkdirs() }
    ...
    private fun ensureBuiltIns() {
        if (registryFile.isFile) {
            ...
            if (current != stored) writeRegistry(current)
            return
        }
        writeRegistry(
            listOf(
