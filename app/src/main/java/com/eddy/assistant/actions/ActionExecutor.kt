...
class ActionExecutor(private val context: Context) {
    ...
    fun openAppByName(requestedName: String): ActionResult {
        val embeddedName = when (requestedName) {
            "EDDY_TOOL_CALCULATOR" -> "calculadora"
            "EDDY_TOOL_STOPWATCH" -> "cronometro"
            "EDDY_TOOL_TIMER" -> "temporizador"
            "EDDY_TOOL_CLOCK" -> "reloj"
            "EDDY_TOOL_NOTES" -> "notas"
            "EDDY_TOOL_CONVERTER" -> "conversor"
            "EDDY_HOME" -> "eddy"
            else -> requestedName
        }
        EddyUiModeStore.resolve(embeddedName)?.let { mode ->
            EddyUiModeStore.set(context, mode)
            val intent = Intent(context, MainActivity::class.java).addFlags(
                Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_SINGLE_TOP or Intent.FLAG_ACTIVITY_CLEAR_TOP,
            )
            val message = if (mode == EddyUiMode.ASSISTANT) {
