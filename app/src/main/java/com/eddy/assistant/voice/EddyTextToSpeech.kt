...
class EddyTextToSpeech(
    ...
    private fun configureNaturalMasculineSpanishVoice() {
        ...
        if (languageResult == TextToSpeech.LANG_MISSING_DATA || languageResult == TextToSpeech.LANG_NOT_SUPPORTED) {
            val fallbacks = listOf("US", "MX", "CR", "CO", "ES").map(::spanishLocale)
    ...
    private fun voiceCountryRank(country: String): Int = when (country.uppercase(Locale.ROOT)) {
        "NI" -> 0; "CR" -> 1; "HN", "SV", "GT" -> 2; "MX", "US" -> 3
        "CO", "VE", "PA" -> 4; "AR", "UY", "CL", "PE", "EC", "BO", "PY", "DO", "PR" -> 5
        "ES" -> 6; else -> 7
    }
    ...
    fun speak(text: String): Boolean {
        ...
        return tts.speak(text, TextToSpeech.QUEUE_FLUSH, null, "eddy_reply_${System.nanoTime()}") == TextToSpeech.SUCCESS
    }
