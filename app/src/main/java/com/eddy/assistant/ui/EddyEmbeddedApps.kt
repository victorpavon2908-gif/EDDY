...
@Composable
fun EddyEmbeddedApp(mode: EddyUiMode, onHome: () -> Unit) {
    when (mode) {
        EddyUiMode.CALCULATOR -> CalculatorApp(onHome)
        EddyUiMode.STOPWATCH -> StopwatchApp(onHome)
        EddyUiMode.TIMER -> TimerApp(onHome)
        EddyUiMode.CLOCK -> ClockApp(onHome)
        EddyUiMode.NOTES -> NotesApp(onHome)
        EddyUiMode.CONVERTER -> ConverterApp(onHome)
        EddyUiMode.ASSISTANT -> Unit
    }
}
